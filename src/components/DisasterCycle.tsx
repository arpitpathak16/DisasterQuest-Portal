...
const DisasterCycle = () => {
  ...
  const cycleSegments: CycleSegment[] = [
    {
      ...
      name: "Preparation",
      description:
        "Get ready before disasters strike by learning preparation techniques.",
      color: "bg-preimpact",
      icon: Clock,
      points: 100,
    },
    {
      ...
      name: "Response",
      description:
        "Learn how to respond effectively during emergency situations.",
      color: "bg-emergency",
      icon: ShieldAlert,
      points: 150,
    },
    {
      ...
      name: "Restoration",
      description:
        "Help communities restore essential services after disasters.",
    ...
    },
    {
      ...
      name: "Recovery",
      description:
        "Rebuild and recover from disaster impacts for long-term sustainability.",
      color: "bg-recovery",
      icon: Home,
      points: 100,
    },
    {
      ...
      name: "Mitigation",
      description:
        "Reduce or prevent future disaster risks through strategic planning.",
      color: "bg-mitigation",
      icon: Building,
      points: 175,
    },
    {
      ...
      description:
        "Develop community resilience and preparedness for future events.",
      color: "bg-capacity",
      icon: Users,
      points: 200,
    },
  ...
  const startChallenge = (id: string) => {
    if (!user) {
      toast({
        ...
        description:
          "Please sign in to start challenges and track your progress.",
        variant: "destructive",
      });
      return;
    }

    // Find a challenge for this category
    const categoryChallenges = challenges.filter(
      (challenge) => challenge.category.toLowerCase() === id
    );

    if (categoryChallenges.length > 0) {
      // Navigate to the first challenge for this category
      navigate(`/challenge/${categoryChallenges[0].id}`);
    } else {
      toast({
        title: "No Challenges Available",
        description: "There are no challenges available for this category yet.",
        variant: "destructive",
      });
    }
  };
  ...
  const isSegmentCompleted = (segmentId: string): boolean => {
    ...
    // Check if any challenge in this category is completed
    const categoryChallenges = challenges.filter(
      (challenge) => challenge.category.toLowerCase() === segmentId
    );
    return categoryChallenges.some((challenge) =>
          ...
          <div className="grid grid-cols-2 grid-rows-3 gap-2 h-full">
            {cycleSegments.map((segment) => (
              <div
                ...
                className={`${
                  segment.color
                } rounded-lg p-4 flex flex-col items-center justify-center cursor-pointer transition-all duration-300 hover:scale-105 ${
              ...
              <div key={segment.id} className="space-y-4">
                ...
                <div className="flex justify-between items-center pt-4">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-medium">{segment.points} points</span>
                  </div>
                  <Button
                    onClick={() => startChallenge(segment.id)}
                    className="gap-1">
