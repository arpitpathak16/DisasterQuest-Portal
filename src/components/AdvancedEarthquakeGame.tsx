...
export default function AdvancedEarthquakeGame() {
  const gridSize = 15;
  ...
  const movePlayer = (dx: number, dy: number) => {
    console.log(`Attempting to move player by dx: ${dx}, dy: ${dy}`);
    setPlayerPosition((pos) => {
      ...
      if (tile?.isObstacle && !powerUpActive) return pos;
  ...
  const activatePowerUp = () => {
    ...
    setMessage("💥 Power-Up Activated! Move through obstacles!");
  ...
  const nextLevel = () => {
    setLevel((lvl) => lvl + 1);
    resetGame();
    setMessage(`Level ${level + 1} - More challenges await!`);
  };

  return (
    <div className="p-4 max-w-5xl mx-auto text-center">
      <h1 className="text-3xl font-bold mb-2">
        Advanced QuakeSafe: Navigate the Grid
      </h1>
      ...
      <button
        ...
        className="mt-4 px-4 py-2 bg-yellow-600 text-white font-semibold rounded-xl shadow">
        ⚡ Activate Power-Up
      </button>
