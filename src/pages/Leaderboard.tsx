...
import { Trophy, Search, Medal, User as UserIcon, BadgeCheck, RefreshCw } from 'lucide-react';
import { Tabs, TabsContent, TabsList, TabsTrigger } from '@/components/ui/tabs';
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '@/components/ui/card';
import { useUser } from '@/contexts/UserContext';
import CollaborationService, { LeaderboardEntry } from '@/services/CollaborationService';
...
const Leaderboard = () => {
  ...
  const [leaderboardData, setLeaderboardData] = useState<LeaderboardEntryWithRank[]>([]);
  ...
  useEffect(() => {
    ...
    CollaborationService.subscribe('leaderboard-updated', handleLeaderboardUpdate);

    return () => {
      CollaborationService.unsubscribe('leaderboard-updated', handleLeaderboardUpdate);
    };
  }, [user]);

  const refreshLeaderboardData = () => {
    setIsLoading(true);
    const data = CollaborationService.getLeaderboard();
    processLeaderboardData(data);
    setIsLoading(false);
  };
          ...
          <div className="mb-12">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {getTopThree().map((entry) => (
                <Card 
                  key={entry.rank}
                  className={`${entry.rank === 1 ? 'md:order-2 md:scale-110 z-10' : entry.rank === 2 ? 'md:order-1' : 'md:order-3'} ${entry.isCurrentUser ? 'border-primary' : ''}`}
                >
                  <CardHeader className="text-center pb-2">
                    <div className={`w-12 h-12 rounded-full ${getTopThreeColor(entry.rank)} mx-auto flex items-center justify-center`}>
                      {entry.rank === 1 ? (
                  ...
                  <CardContent className="text-center">
                    ...
                    <div className="flex justify-center gap-6 mt-4">
                      ...
                      <div className="text-center">
                        <p className="text-lg font-medium">{entry.challenges}</p>
                    ...
                    <Table>
                      ...
                      <TableBody>
                        {filteredData.map((entry) => (
                          <TableRow key={entry.id} className={entry.isCurrentUser ? 'bg-primary/5' : ''}>
                            <TableCell className="font-medium">
                              {entry.rank <= 3 ? (
                                <div className={`w-6 h-6 rounded-full ${getTopThreeColor(entry.rank)} flex items-center justify-center`}>
                                  <span className="text-white text-xs">{entry.rank}</span>
                                </div>
                              ) : (
                                entry.rank
                              )}
                ...
                <Card>
                  <CardContent className="p-0">
                    <Table>
                      ...
                      <TableBody>
                        {[...filteredData].sort((a, b) => b.badges - a.badges).map((entry, index) => (
                          <TableRow key={entry.id} className={entry.isCurrentUser ? 'bg-primary/5' : ''}>
                ...
                <Card>
                  <CardContent className="p-0">
                    <Table>
                      ...
                      <TableBody>
                        {[...filteredData].sort((a, b) => b.challenges - a.challenges).map((entry, index) => (
                          <TableRow key={index} className={entry.isCurrentUser ? 'bg-primary/5' : ''}>
                            <TableCell className="font-medium">{index + 1}</TableCell>
                            <TableCell>
                              <div className="flex items-center gap-2">
                                <div className="bg-muted w-8 h-8 rounded-full flex items-center justify-center">
                            ...
                            <TableCell className="text-right font-bold">{entry.challenges}</TableCell>
                          </TableRow>
                        ))}
                      </TableBody>
                    </Table>
